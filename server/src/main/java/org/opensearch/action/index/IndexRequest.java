...
public class IndexRequest extends ReplicatedWriteRequest<IndexRequest> implements DocWriteRequest<IndexRequest>, CompositeIndicesRequest {
    ...
    private final Logger logger = Loggers.getLogger(IndexRequest.class, "MOCHI");
    ...
    public void process(Version indexCreatedVersion, @Nullable MappingMetadata mappingMd, String concreteIndex) {
        ...
        if (id == null) {
            ...
            // Associate the selected doc ID (mapping to pre-selected shard)
            id(UUIDs.base64UUID());
            logger.info("Auto-generated Doc ID: {}, Time taken: {}, ", this.id, (System.currentTimeMillis() - startTimeMs));
        }
    }

    public void process(@Nullable MappingMetadata mappingMd, String concreteIndex, String docId) {
        if (mappingMd != null) {
            // might as well check for routing here
            if (mappingMd.routingRequired() && routing == null) {
                throw new RoutingMissingException(concreteIndex, id);
        ...
        if (id == null) {
            ...
            // Associate the selected doc ID (mapping to pre-selected shard)
            id(docId);
            logger.info("Optimized Doc ID: {}, Time taken: {}, ", this.id, (System.currentTimeMillis() - startTimeMs));
        }
    }
